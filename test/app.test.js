...
test('bracket uses connected desktop and dynamic two-column mobile maps', () => {
  ...
  assert.match(app, /function mobileSideDivider\(\)/);
  assert.match(app, /Side A · SF1 path/);
  assert.match(app, /Side B · SF2 path/);
  assert.match(app, /function mobileFinishPath\(\)/);
  assert.match(app, /Side A · SF1<\/span>' \+ visualSlot\('M101'/);
  assert.match(app, /Side B · SF2<\/span>' \+ visualSlot\('M102'/);
  assert.match(app, /visualSlot\('M104', 1, 1, 'bracket-final-node'/);
  assert.match(app, /Confirmed champion/);
  assert.match(app, /Your champion pick/);
  ...
  assert.match(css, /\.bracket-section-tabs \{[\s\S]*?grid-template-columns: repeat\(4, minmax\(0, 1fr\)\)/);
  assert.match(css, /\.bracket-mobile-side-divider/);
  assert.match(css, /\.bracket-mobile-finish-path \{ flex-basis: 180px; min-height: 180px; \}/);
  assert.match(css, /\.bracket-mobile-scroll\[data-mobile-stage-shell="sf"\] \{ flex: 0 0 auto; \}/);
  assert.match(css, /\.bracket-mobile-visual\[data-mobile-stage="sf"\] \{ flex: 0 0 auto; min-height: 0; \}/);
  assert.match(css, /\.bracket-mobile-champion-card \{[\s\S]*?min-height: 36px/);
